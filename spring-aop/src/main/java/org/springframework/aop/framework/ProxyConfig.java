...
public class ProxyConfig implements Serializable {
	...
	//标记是否对代理进行优化。
	// true :如果Bean有接门就直接使用JDK动态代理,没有接口就使用CGLIB
	...
	//默认情况,我们可以这么用Advised target = (Advised) context.getBean("opaqueTest");从而就可以对该代理持有的一些属性进行人为干预
	boolean opaque = false;
	//标记代理对象是否应该被aop框架通过AopContextl以ThreadLocal的形式暴露出去。
	//当一个代理对象需要调用它【proxy本身】的另外一个代理方法时,这个属性将非常有用。默认是是false
